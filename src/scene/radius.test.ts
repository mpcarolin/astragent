...
const look = (radiusKm: number) => ({ color: 0, radiusKm, texture: "" });

describe("radius", () => {
  it("converts kilometres to scaled scene units", () => {
    expect(radius(look(KM_PER_AU))).toBeCloseTo(RADIUS_SCALE, 10);
  });

  it("is linear in the body radius", () => {
    expect(radius(look(2000))).toBeCloseTo(2 * radius(look(1000)), 12);
  });
  ...
  it("gives earth a radius under a tenth of a scene unit", () => {
    ...
    expect(radius(earth)).toBeLessThan(0.1);
    expect(radius(earth)).toBeGreaterThan(0);
  });
