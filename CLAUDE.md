...
- Colocated: `<filename>.test.ts` next to the file it tests, same folder. `test/` holds
  only `fixtures/`.
- Never make source code worse to make it testable.
- When the natural design does I/O a test can't run under the `node` environment (texture
  loading, today), `vi.mock` that one module and keep the mock to plain data. Otherwise no
  mocks: no mocking pure code, no `vi.fn`, no snapshots.
- Expected values come from Horizons fixtures, a defining property, a hand-derived case, or
  a textbook constant.
